...
export default function ProductCard({
  ...
  const handleImageHover = () => {
    if (!product.soldOut && currentColor.images.length > 1) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`flex flex-col group cursor-pointer min-h-[360px] justify-between px-2 ${
        product.soldOut ? "opacity-60 pointer-events-none" : ""
      }`}
    >
      {/* Product Image */}
      <div
            ...
            {currentColor.images.map((_, index) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-colors duration-200 ${
                  currentImageIndex === index ? "bg-black" : "bg-black/30"
                }`}
        ...
        <div className="flex items-center gap-2 mt-1 mb-2">
          {product.colors.slice(0, maxVisibleColors).map((color) => (
            <motion.button
              ...
              } ${product.soldOut ? "cursor-not-allowed opacity-50" : ""}`}
